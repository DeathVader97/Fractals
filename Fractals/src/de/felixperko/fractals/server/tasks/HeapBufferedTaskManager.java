...
import de.felixperko.fractals.server.data.Instance;
...
public class HeapBufferedTaskManager extends FractalsThread implements TaskManager {
	...
	public HeapBufferedTaskManager(GridRenderer renderer) {
		super("TaskManager", 5);
		this.id = ID_COUNTER++;
	...
	Map<Integer, PriorityQueue<Chunk>> openPositions = new HashMap<>(); 
	Map<Integer, PriorityQueue<ChunkTask>> bufferedTasks = new HashMap<>();
	Map<Instance, Map<Integer, Integer>> internalIndices = new HashMap<>();
...
//	public List<ChunkTask> priorityList = new ArrayList<>();
	...
	private void startGeneration(Instance instance, Position startPos, int stateId) {
		Integer internalIndex = getInternalIndex(instance, stateId);
		boolean done = false;
		PriorityQueue<Chunk> open = openPositions.get(internalIndex);
		PriorityQueue<ChunkTask> buff = bufferedTasks.get(internalIndex);
		while (!done && buff.size() < buffer_size) {
			Chunk c = open.poll();
			for (Position neighbourPos  : c.getNeighbourPositions()) {
				Chunk nc = grid.getChunk(neighbourPos);
				if (!nc.isMarked(id)) {
					open.add(nc);
					nc.setMarked(id);
				}
	...
	private boolean addInternalIndex(Instance instance, Integer chunkState) {
		Map<Integer, Integer> instanceMap = internalIndices.get(instance);
		if (instanceMap == null) {
			instanceMap = new HashMap<>();
			internalIndices.put(instance, instanceMap);
		}
		if (instanceMap.containsKey(chunkState))
			return false;
		Integer index = INDEX_COUNTER++;
		instanceMap.put(chunkState, index);
		openPositions.put(index, new PriorityQueue<Chunk>());
		bufferedTasks.put(index, new PriorityQueue<ChunkTask>());
		return true;
	}
	
	private boolean removeInstanceInternalIndices(Instance instance) {
		Map<Integer, Integer> instanceMap = internalIndices.get(instance);
		if (instanceMap == null)
			return false;
		List<Integer> stateIds = new ArrayList<>(instanceMap.keySet());
		removeInternalIndex(instanceMap, stateId);
		for (Integer stateId : stateIds) {
		}
		internalIndices.remove(instance);
		return true;
	}
	

	private boolean removeInternalIndex(Instance instance, Integer chunkState) {
		Map<Integer, Integer> instanceMap = internalIndices.get(instance);
		if (instanceMap == null)
			return false;
		return removeInternalIndex(instanceMap, chunkState);
	}
	
	private boolean removeInternalIndex(Map<Integer, Integer> instanceMap, Integer stateId) {
		Integer internalId = instanceMap.remove(stateId);
		if (internalId == null)
			return false;
		//TODO dispose of tasks necessary?
		bufferedTasks.remove(internalId);
		openPositions.remove(internalId);
		return true;
	}
	
	private int getInternalIndex(Instance instance, Integer chunkState) {
		Integer i = internalIndices.get(instance).get(chunkState);
		if (i == null) {
			addInternalIndex(instance, chunkState);
			return internalIndices.get(instance).get(chunkState);
		}
		return i;
	}
